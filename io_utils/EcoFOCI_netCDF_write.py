...
class NetCDF_Create_Profile_Ragged1D(object):
    ...
    def variable_init(self, EPIC_VARS_dict, verbose=False):
        ...
        rec_var_type= ['f4'] + ['f8' for spot in rec_vars[1:]]
    ...
    def add_data(self, EPIC_VARS_dict, data_dic=None, missing_values=99999, pandas=False):
        """
        ...
        if pandas:
            for EPICdic_key in EPIC_VARS_dict.keys():
                print("Adding {0}".format(EPICdic_key))
                di = self.rec_vars.index(EPICdic_key)
                try:
                    self.var_class[di][:] = data_dic[EPICdic_key].values
                except KeyError:
