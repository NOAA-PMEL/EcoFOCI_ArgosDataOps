...
def read_drifter_region(db, cursor, table, region):
    sql = ("SELECT * from `{0}` WHERE `DeploymentWaters`= '{1}' AND `IncludeInMovie` = 'y'").format(table, region)
...
def read_drifter_region_year(db, cursor, table, region, year):
    sql = ("SELECT * from `{0}` WHERE `DeploymentWaters`= '{1}' AND `ReleaseDate` BETWEEN '{2}-01-01' AND '{3}-12-30'").format(table, region, year, year+1)
    result_dic = {}
    try:
        # Execute the SQL command
        cursor.execute(sql)
...
parser.add_argument('year', metavar='year', type=int, help='year of deployment')
parser.add_argument('--kml', action="store_true", help='output as kml')
parser.add_argument('--png', action="store_true", help='output as png')
parser.add_argument('--vecdis', action="store_true", help='read a vecdis file')
parser.add_argument('--yearfile', action="store_true", help='read a .y2014 file')
...
### Sets up region specific contour intervals for topography
if args.all_region.lower() == 'arctic':
...
if args.all_region and args.all_movie_img:
    ...
    table = 'drifter_ids_pre2015'
    drifterIDs = read_drifter_region_year(db, cursor, table, region, args.year)
    close_DB(db)

    for tdoy in range(0,366,1):
        ...
        for ind_num, k in enumerate(drifterIDs.keys()):
            ...
            args.year = args.year
            file_date = (datetime.timedelta(tdoy - 1) + datetime.datetime(args.year,1,1)).strftime('%Y-%m-%d')
            print file_date
            isemptydata = True
            drifter_data = {}
            ...
            try:
                ...
                doy = [float(x[4]) for x in drifter_data['DATA']] 
                doy = np.array(doy)
                sub_year_ind = (year >= args.year)
                sub_doy_ind = (doy >= tdoy -5.) & ( year >= args.year)
                nodata = False
            except:
                nodata = True
